...
| **12B** | 12B | Dense, unified (encoder-free) | **256K** | Newest addition to the family (added ~2 months after initial launch) <cite>turn5search98</cite> |
...
Official Google source for context windows and architecture: <cite>turn5search47</cite><cite>turn5search52</cite>
...
though see the Known Issues section below on reliability. <cite>turn5search47</cite>
...
BF16 than a standard post-training-quantized (PTQ) Q4 of the same size. <cite>turn5search96</cite>
...
release scope; <cite>turn5search96</cite> the per-model VRAM figures above are
corroborated across independent QAT summaries and an AMD 7900 XTX benchmark showing
a 5.7GB VRAM reduction for 12B QAT vs. Q8_0. <cite>turn5search97</cite><cite>turn5search100</cite><cite>turn5search98</cite>
...
losing the quality QAT was meant to preserve. <cite>turn5search100</cite>

## 3. LM Studio Load Parameters (set at model load, requires reload to change)

Official parameter names from LM Studio's API reference: <cite>turn5search66</cite><cite>turn5search106</cite>

| Parameter | Recommended value | Why |
|---|---|---|
| `contextLength` | 32,768 minimum; up to 256K if VRAM allows | Matches your stated 32K floor; Gemma 4 12B+ natively supports up to 256K <cite>turn5search47</cite> |
| `flashAttention` | `true` | Reduces memory use, speeds generation on compatible hardware (RTX 5060 Ti supports it) <cite>turn5search66</cite> |
| `useFp16ForKVCache` | `true` (or leave KV cache unquantized per Cline's own stack guide) | Cline's official local-stack blog explicitly recommends leaving KV Cache Quantization **unchecked** when pairing with Cline <cite>turn5search73</cite> |
| `gpu` (offload ratio) | Max your VRAM allows | Full GPU offload where possible |
| `evalBatchSize` | Default, raise only if VRAM headroom exists | Trades memory for throughput <cite>turn5search66</cite> |
| `keepModelInMemory` | `true` if RAM allows | Avoids reload latency between tasks |
| `num_experts` | Default for 26B-A4B (MoE only) | Only applies to the MoE variant <cite>turn5search67</cite> |
...
contexts. <cite>turn5search65</cite> **(community/unverified for your exact hardware — benchmark before committing to a large context in production use.)**

## 4. Sampling / Inference Parameters

**Official Gemma 4 defaults**, as shipped in the Ollama model library params.json
for the Gemma 4 family: <cite>turn5search88</cite><cite>turn5search89</cite>
...
|---|---|---|
| `temperature` | 0.2–0.3 | Community consensus across multiple independent guides <cite>turn5search102</cite><cite>turn5search105</cite> |
| `top_p` | 0.85–0.9 | Same |
| `top_k` | 40 | Same |
...
conflicts with the general-purpose coding guidance above. <cite>turn5search87</cite>
...
at the model level, not something you configure separately. <cite>turn5search47</cite>

In LM Studio: switch to **Power User mode**, load your main model, then select a
**Draft Model** in the Speculative Decoding section of the chat sidebar. <cite>turn5search90</cite>
General guidance for draft-model sizing (applies if you pair main 31B with a smaller
Gemma 4 variant as draft, e.g. E4B): <cite>turn5search90</cite>
...
chat template using `--jinja --chat-template-file` flags. <cite>turn5search98</cite>
...
guaranteed-parseable output rather than relying on prompt instructions alone. <cite>turn5search104</cite>
